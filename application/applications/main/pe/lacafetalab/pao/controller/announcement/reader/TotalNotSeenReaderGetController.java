...
import pe.lacafetalab.pao.communication.announcement.application.reader.total_not_seen.GetReaderNotSeenQuery;
import pe.lacafetalab.pao.communication.announcement.application.reader.total_not_seen.NotSeenResponse;
...
@RestController
public final class TotalNotSeenReaderGetController extends ApiController {
    public TotalNotSeenReaderGetController(QueryBus queryBus, CommandBus commandBus) {
        super(queryBus, commandBus);
    }

    @GetMapping("/announcement/reader/{id}/total-not-seen")
    public ResponseEntity<RestResponse> index(@PathVariable String id) throws QueryHandlerExecutionError, QueryNotRegisteredError {
        ...
        NotSeenResponse notSeenResponse = ask(new GetReaderNotSeenQuery(idDemo));
        return RestResponse.data(notSeenResponse);
    }
